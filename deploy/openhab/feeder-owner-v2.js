...
// This unlinked String is receipt state only: no command trigger or autoupdate.
const LEDGER_ITEM = 'GoatFeeder_OwnerLedgerV2';
...
function validLedgerEntry(entry) {
  return Boolean(
    ...
    && (entry.status !== 'complete' || typeof entry.updatedAt === 'string')
    && (entry.updatedAt === undefined || (
      typeof entry.updatedAt === 'string' && Number.isFinite(epochMillis(entry.updatedAt))
      && epochMillis(entry.updatedAt) >= epochMillis(entry.at)
    ))
  );
}

function parseLedger(ledgerItem) {
  const raw = ledgerItem.state.toString();
  ...
  const persisted = ledgerItem.persistence.previousState(false, 'jdbc');
...
function writeLedger(ledgerItem, ledger, requestId, expectedStatus) {
  const encoded = JSON.stringify({
    version: LEDGER_VERSION,
    entries: ledger.entries,
  });
  if (ledger.entries.length > MAX_LEDGER_ENTRIES || utf8Bytes(encoded) > MAX_LEDGER_BYTES) throw new Error('ledger_invalid');
  ledgerItem.postUpdate(encoded);
  // postUpdate is asynchronous too. Never ask JDBC to persist the old state
  // merely because one fixed sleep elapsed. Ownership remains held on timeout.
  let updated = false;
  for (let attempt = 0; attempt < LEDGER_READBACK_ATTEMPTS; attempt += 1) {
    settleItemUpdate();
    if (ledgerItem.state.toString() === encoded) { updated = true; break; }
  }
  if (!updated) throw new Error('ledger_readback_failed');
  ledgerItem.persistence.persist('jdbc');

  for (let attempt = 0; attempt < LEDGER_READBACK_ATTEMPTS; attempt += 1) {
    try {
      const readback = JSON.parse(ledgerItem.state.toString());
      const previous = ledgerItem.persistence.previousState(false, 'jdbc');
...
function runFeederOwner(triggerEvent) {
  ...
  try {
    ...
    ledgerItem = items.getItem(LEDGER_ITEM);
    ledger = parseLedger(ledgerItem);
    ...
    // Admission precedes asynchronous persistence and is not the ON time.
    // Completion is a conservative durable bound after every prior ON; use all
    // retained receipts so restored ordering cannot shorten the start interval.
    const durableStart = Math.max(-Infinity, ...ledger.entries.map(e => epochMillis(e.updatedAt)));
  ...
  try { writeLedger(ledgerItem, ledger, request.requestId, 'accepted'); }
  catch {
  ...
  try {
    ...
    const actualStart = nowMillis();
    if (!Number.isFinite(actualStart) || actualStart < currentMs) throw new Error('clock_invalid');
    cache.shared.put(LAST_START_KEY, String(actualStart));
    ...
    actions.ScriptExecution.createTimer(now().plusSeconds(1), () => {
      ...
      try { writeLedger(ledgerItem, complete, request.requestId, 'complete'); }
      catch {
