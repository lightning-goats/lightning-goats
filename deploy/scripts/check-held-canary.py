...
its one request. On interruption use control-held-canary --release with that UUID;
...
def check(env, evidence=None):
    ...
    with os.fdopen(fd,'w') as out:
        def record(stage):
            report['stage']=stage;out.seek(0);out.truncate();json.dump(report,out,indent=2);out.flush();os.fsync(out.fileno())
        record('prepared')
